...
<form method="POST" action="{{ route('matakuliah.update', $matkul->id) }}">
    ...
    <div class="mb-1 mx-auto form-admin">
        ...
        <input type="time" name="awal_seleksi" value="{{ $matkul->awal_seleksi }}" class="form-control @error('awal_seleksi') is-invalid @enderror" required>
        @error('awal_seleksi')
        <div class="alert alert-danger mt-1">{{ $message }}</div>
        @enderror
    </div>
    <div class="mb-1 mx-auto form-admin">
        ...
        <input type="time" name="akhir_seleksi" value="{{ $matkul->akhir_seleksi }}" class="form-control @error('akhir_seleksi') is-invalid @enderror" required>
        @error('akhir_seleksi')
        <div class="alert alert-danger mt-1">{{ $message }}</div>
        @enderror
    </div>
